...
FastMapMatchConfig::FastMapMatchConfig( int k_arg, double r_arg, double gps_error, double reverse_tolerance ) : k( k_arg ), radius( r_arg ), gps_error( gps_error ), reverse_tolerance( reverse_tolerance ) {};

void FastMapMatchConfig::print() const
{
    SPDLOG_INFO( "FMMAlgorithmConfig" );
    SPDLOG_INFO( "k {} radius {} gps_error {} reverse_tolerance {}", k, radius, gps_error, reverse_tolerance );
};

FastMapMatchConfig FastMapMatchConfig::load_from_xml( const boost::property_tree::ptree& xml_data )
{
...
FastMapMatchConfig FastMapMatchConfig::load_from_arg( const cxxopts::ParseResult& arg_data )
{
...
{
    options.add_options()( "k,candidates", "Number of candidates", cxxopts::value< int >()->default_value( "8" ) )( "r,radius", "Search radius", cxxopts::value< double >()->default_value( "300.0" ) )(
        "reverse_tolerance", "Ratio of reverse movement allowed", cxxopts::value< double >()->default_value( "0.0" ) )( "e,error", "GPS error", cxxopts::value< double >()->default_value( "50.0" ) );
}
...
{
    if ( gps_error <= 0 || radius <= 0 || k <= 0 || reverse_tolerance < 0 || reverse_tolerance > 1 )
    {
        SPDLOG_CRITICAL( "Invalid mm parameter k {} r {} gps error {} reverse_tolerance {}", k, radius, gps_error, reverse_tolerance );
        return false;
    }
    return true;
}

MatchResult FastMapMatch::match_traj( const Trajectory& traj, const FastMapMatchConfig& config )
{
    ...
    std::transform( tg_opath.begin(), tg_opath.end(), matched_candidate_path.begin(),
                    []( const TGNode* a )
                    {
    ...
    C_Path                     cpath = ubodt_->construct_complete_path( traj.id, tg_opath, edges, &indices, config.reverse_tolerance );
...
PyMatchResult FastMapMatch::match_wkt( const std::string& wkt, const FastMapMatchConfig& config )
{
    ...
    for ( int i = 0; i < result.opt_candidate_path.size(); ++i )
    {
        ...
        output.candidates.push_back( { i, mc.c.edge->id, graph_.get_node_id( mc.c.edge->source ), graph_.get_node_id( mc.c.edge->target ), mc.c.dist, mc.c.offset, mc.c.edge->length, mc.ep, mc.tp, mc.sp_dist } );
        output.pgeom.add_point( mc.c.point );
    }
    return output;
};

std::string FastMapMatch::match_gps_file( const FMM::CONFIG::GPSConfig& gps_config, const FMM::CONFIG::ResultConfig& result_config, const FastMapMatchConfig& fmm_config, bool use_omp )
{
...
double FastMapMatch::get_sp_dist( const Candidate* ca, const Candidate* cb, double reverse_tolerance )
{
    double sp_dist = 0;
...
void FastMapMatch::update_tg( TransitionGraph* tg, const Trajectory& traj, double reverse_tolerance )
{
    ...
    for ( int i = 0; i < N - 1; ++i )
    {
        ...
        update_layer( i, &( layers[ i ] ), &( layers[ i + 1 ] ), eu_dists[ i ], reverse_tolerance, &connected );
        if ( ! connected )
        {
...
void FastMapMatch::update_layer( int level, TGLayer* la_ptr, TGLayer* lb_ptr, double eu_dist, double reverse_tolerance, bool* connected )
{
    ...
    for ( auto iter_a = la_ptr->begin(); iter_a != la_ptr->end(); ++iter_a )
    {
        ...
        for ( auto iter_b = lb_ptr->begin(); iter_b != lb_ptr->end(); ++iter_b )
        {
            ...
            SPDLOG_TRACE( "L {} f {} t {} sp {} dist {} tp {} ep {} fcp {} tcp {}", level, iter_a->c->edge->id, iter_b->c->edge->id, sp_dist, eu_dist, tp, iter_b->ep, iter_a->cumu_prob, temp );
