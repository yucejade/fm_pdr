...
{
    SPDLOG_TRACE( "Intialization UBODT with buckets {} multiplier {}", buckets, multiplier );
...
std::vector< EdgeIndex > UBODT::look_sp_path( NodeIndex source, NodeIndex target ) const
{
...
C_Path UBODT::construct_complete_path( int traj_id, const TGOpath& path, const std::vector< Edge >& edges, std::vector< int >* indices, double reverse_tolerance ) const
{
    C_Path cpath;
    if ( ! indices->empty() )
        indices->clear();
    ...
    for ( int i = 0; i < N - 1; ++i )
    {
        ...
        if ( ( a->edge->id != b->edge->id ) || ( a->offset - b->offset > a->edge->length * reverse_tolerance ) )
        {
            ...
            if ( segs.empty() && a->edge->target != b->edge->source )
            {
                ...
                SPDLOG_DEBUG( "reverse movement {} tolerance {}", a->offset - b->offset, a->edge->length * reverse_tolerance );
                SPDLOG_WARN( "Traj {} unmatched as edge {} L {} offset {}"
                             " and edge {} L {} offset {} disconnected",
                             traj_id, a->edge->id, a->edge->length, a->offset, b->edge->id, b->edge->length, b->offset );
...
long UBODT::estimate_ubodt_rows( const std::string& filename )
{
    ...
    if ( rc == 0 )
    {
        ...
        std::transform( fn_extension.begin(), fn_extension.end(), fn_extension.begin(),
                        []( unsigned char c )
                        {
...
{
    std::vector< int > prime_numbers = { 5003, 10039, 20029, 50047, 100669, 200003, 500000, 1000039, 2000083, 5000101, 10000103, 20000033 };
...
std::shared_ptr< UBODT > UBODT::read_ubodt_file( const std::string& filename, int multiplier )
{
...
std::shared_ptr< UBODT > UBODT::read_ubodt_csv( const std::string& filename, int multiplier )
{
    ...
    while ( fgets( line, BUFFER_LINE, stream ) )
    {
        ...
        sscanf( line, "%d;%d;%d;%d;%d;%lf", &r->source, &r->target, &r->first_n, &r->prev_n, &r->next_e, &r->cost );
        r->next = nullptr;
        table->insert( r );
...
std::shared_ptr< UBODT > UBODT::read_ubodt_binary( const std::string& filename, int multiplier )
{
