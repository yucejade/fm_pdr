...
CFmtDeviceWrapper::CFmtDeviceWrapper()
{
    ...
    // Accel ODR = 50 Hz and Full Scale Range = 16G
    sensor_imu.startAccel( 50, 16 );
    // Gyro ODR = 50 Hz and Full Scale Range = 2000 dps
    sensor_imu.startGyro( 50, 2000 );
...
bool CFmtDeviceWrapper::ReadData( PDRSensorData& sensor_data, int index )
{
    ...
    sensor_data.mag_time[index] = timestamp;
    sensor_data.mag_x[ index ] = x;
    sensor_data.mag_y[ index ] = y;
    sensor_data.mag_z[ index ] = z;

    // TDK42607
    inv_imu_sensor_event_t imu_event;
    sensor_imu.getDataFromRegisters( imu_event );

    sensor_data.acc_time[index] = timestamp;
    sensor_data.acc_x[ index ] = imu_event.accel[ 0 ] / 2048.0;
    sensor_data.acc_y[ index ] = imu_event.accel[ 1 ] / 2048.0;
    sensor_data.acc_z[ index ] = imu_event.accel[ 2 ] / 2048.0;
    sensor_data.gyr_time[index] = timestamp;
    sensor_data.gyr_x[ index ] = imu_event.gyro[ 0 ] / 16.4;
    sensor_data.gyr_y[ index ] = imu_event.gyro[ 1 ] / 16.4;
    sensor_data.gyr_z[ index ] = imu_event.gyro[ 2 ] / 16.4;
